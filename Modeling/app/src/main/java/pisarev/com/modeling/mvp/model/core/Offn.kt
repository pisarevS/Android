...
class Offn : GCode() {
    ...
    private fun radiusCompensationArcOffn(
        ...
        pEnd.x = frameList[numberLIne].x.toFloat()
        pEnd.z = frameList[numberLIne].z.toFloat()
        val radius = frameList[numberLIne].cr
        val offn = frameList[numberLIne].offn
        val chord = sqrt((pStart.x - pEnd.x).pow(2.0F) + (pStart.z - pEnd.z).pow(2.0F))
        var h = sqrt(radius * radius - chord / 2 * (chord / 2)).toFloat()
        if (java.lang.Double.isNaN(h.toDouble())) h = 0.0F
        if (java.lang.Double.isNaN(chord.toDouble())) h = 0.0F
        if (clockwise == 2 && frameList[numberLIne].offn > 0) {
            ...
            if (isToolRadiusCompensation == 1) {
                calculateToolRadiusCompensationClockwise(frameList, numberLIne, radius, offn, x01.toDouble(), z01.toDouble(), G41)
            } //G41
            if (isToolRadiusCompensation == 2) {
                calculateToolRadiusCompensationClockwise(frameList, numberLIne, radius, offn, x01.toDouble(), z01.toDouble(), G42)
            } //G42
        }
        if (clockwise == 3 && frameList[numberLIne].offn > 0) {
            ...
            if (isToolRadiusCompensation == 1) {
                calculateToolRadiusCompensationClockwise(frameList, numberLIne, radius, offn, x02.toDouble(), z02.toDouble(), G42)
            } //G41
            if (isToolRadiusCompensation == 2) {
                calculateToolRadiusCompensationClockwise(frameList, numberLIne, radius, offn, x02.toDouble(), z02.toDouble(), G41)
            } //G42
        }
        pStart.x = pEnd.x
        pStart.z = pEnd.z
    ...
    private fun radiusCompensationLineOffn(frameList: List<Frame>, numberLIne: Int, isToolRadiusCompensation: Int) {
        ...
        if (isToolRadiusCompensation == 1) {
            if (pStart.z == pEnd.z && pStart.x > pEnd.x) {
                if (!containsG41G42(frameList[numberLIne].gCodes)) {
                    ...
                    } else frameList[numberLIne - 1].x = pStart.x.toDouble()
                    frameList[numberLIne - 1].z = pStart.z + offn
                }
                frameList[numberLIne].x = pEnd.x.toDouble()
            ...
            if (pStart.x == pEnd.x && pStart.z > pEnd.z) {
                if (!containsG41G42(frameList[numberLIne].gCodes)) {
                    frameList[numberLIne - 1].x = pStart.x - offn
                    frameList[numberLIne - 1].z = pStart.z.toDouble()
                }
                frameList[numberLIne].x = pEnd.x - offn
                frameList[numberLIne].z = pEnd.z.toDouble()
            } //X==X -Z
            if (pStart.x == pEnd.x && pStart.z < pEnd.z) {
                if (!containsG41G42(frameList[numberLIne].gCodes)) {
                    frameList[numberLIne - 1].x = pStart.x + offn
            ...
            if (pStart.x < pEnd.x && pStart.z > pEnd.z) {
                var angle = Point2D(pEnd.x - pStart.x, pStart.z - pEnd.z).angle(pStart.x, 0.0F)
                angle = 180 - 90 - angle
                val cathet1 = offn * sin(Math.toRadians(angle.toDouble()))
            ...
            if (pStart.x > pEnd.x && pStart.z > pEnd.z) {
                var angle = Point2D(pStart.x - pEnd.x, pStart.z - pEnd.z).angle(pEnd.x, 0.0F)
                angle = 180 - 90 - angle
                val cathet1 = offn * sin(Math.toRadians(angle.toDouble()))
                val cathet2 = sqrt(offn * offn - cathet1 * cathet1)
                if (!containsG41G42(frameList[numberLIne].gCodes)) {
        ...
        if (isToolRadiusCompensation == 2) {
            if (pStart.z == pEnd.z && pStart.x > pEnd.x) {
                if (!containsG41G42(frameList[numberLIne].gCodes)) {
                    frameList[numberLIne - 1].x = pStart.x.toDouble()
                    frameList[numberLIne - 1].z = pStart.z - offn
                }
                frameList[numberLIne].x = pEnd.x.toDouble()
                frameList[numberLIne].z = pEnd.z - offn
